package com.github.albertosh.flitetrakr.sync;
...
import com.github.albertosh.flitetrakr.sync.usecases.connectionsbelowprice.ConnectionsBelowPriceUseCase;
import com.github.albertosh.flitetrakr.sync.usecases.connectionsbelowprice.ConnectionsBelowPriceUseCaseError;
import com.github.albertosh.flitetrakr.sync.usecases.connectionsbelowprice.ConnectionsBelowPriceUseCaseInput;
import com.github.albertosh.flitetrakr.sync.usecases.connectionsbelowprice.IConnectionsBelowPriceUseCase;
...
import com.github.albertosh.flitetrakr.sync.usecases.connectionwithmaximumstops.ConnectionWithMaximumStopUseCase;
import com.github.albertosh.flitetrakr.sync.usecases.connectionwithmaximumstops.ConnectionWithMaximumStopUseCaseInput;
import com.github.albertosh.flitetrakr.sync.usecases.connectionwithmaximumstops.IConnectionWithMaximumStopsUseCase;
import com.github.albertosh.flitetrakr.sync.usecases.priceofconnection.IPriceOfConnectionUseCase;
import com.github.albertosh.flitetrakr.sync.usecases.priceofconnection.PriceOfConnectionUseCase;
import com.github.albertosh.flitetrakr.sync.usecases.priceofconnection.PriceOfConnectionUseCaseError;
import com.github.albertosh.flitetrakr.sync.usecases.priceofconnection.PriceOfConnectionUseCaseInput;
...
public class Main {
    ...
    private static void initConnections() {
        ...
        for (String c : connections) {
            connectionService.addConnection(Connection.fromString(c.trim()));
        }
    }
    ...
    private static void executePriceOfConnectionUseCase(String inputString) {
        ...
        if (!matcher.matches()) {
            ...
            try {
                Integer output = priceOfConnectionUseCase.execute(input);
    ...
    private static void executeCheapestConnectionUseCase(String inputString) {
        ...
        if (!matcher.matches()) {
            ...
            try {
                CheapestConnectionUseCaseOutput output = cheapestConnectionUseCase.execute(input);
                System.out.println(
                        String.join("-", output.getCities())
                                + "-"
                                + output.getPrice()
                );
    ...
    private static void executeMaximumStopsConnectionUseCase(String inputString) {
        ...
        if (!matcher.matches()) {
            ...
            Integer output = connectionWithMaximumStopsUseCase.execute(input);
            System.out.println(output);
        }
    }
    ...
    private static void executeExactStopsConnectionUseCase(String inputString) {
        ...
        if (!matcher.matches()) {
            ...
            Integer output = connectionWithExactStopUseCase.execute(input);
            System.out.println(output);
        }
    }
